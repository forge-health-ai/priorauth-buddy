...
export function BuddyMascot({ mood = 'happy', size = 120, onPress }: BuddyMascotProps) {
  ...
  return (
    <AnimatedPressable style={animatedStyle} onPress={handlePress}>
      <Svg width={size} height={size * 0.85} viewBox={`0 0 ${size} ${size * 0.85}`}>
        ...
        {getEyes()}
        {getMouth()}
        <Circle cx={size * 0.3} cy={size * 0.52} r={size * 0.035} fill="#FF6B35" opacity={0.3} />
        <Circle cx={size * 0.7} cy={size * 0.52} r={size * 0.035} fill="#FF6B35" opacity={0.3} />
      </Svg>
    </AnimatedPressable>
  );
}
